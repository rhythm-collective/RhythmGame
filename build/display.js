...
// TODO: Display unknown note types as question marks
class NoteManager {
    constructor(tracks_, initialTime) {
        this.tracks = tracks_;
    ...
    drawAllNotes(leastTime, greatestTime) {
        for (let i = 0; i < this.tracks.length; i++) {
            this.drawNotesInTrack(leastTime, greatestTime, this.tracks[i], i, this.tracks.length, this.currentTime);
        }
    }
    drawNotesInTrack(leastTime, greatestTime, track, trackNumber, numTracks, currentTime) {
    ...
    getFirstAndLastNotes(leastTime, greatestTime, track) {
        let i;
    ...
    getGreatestTime(leastTime) {
        return leastTime + canvas.height * this.secondsPerPixel;
    }
    getNoteX(trackNumber, numTracks) {
        let noteTrackSize = canvas.width / (numTracks + (numTracks + 1) / 2);
        return (0.5 + trackNumber * 1.5) * noteTrackSize;
    }
    getNoteY(noteTime, currentTime) {
        let timeDistance = noteTime - currentTime;
        return timeDistance / this.secondsPerPixel;
    }
    drawAllConnectors(leastTime, greatestTime) {
        for (let i = 0; i < this.tracks.length; i++) {
            this.drawConnectorsInTrack(leastTime, greatestTime, this.tracks[i], i, this.tracks.length, this.currentTime);
        }
    }
    drawConnectorsInTrack(leastTime, greatestTime, track, trackNumber, numTracks, currentTime) {
        let noteStack = [];
